...
function buildSentence(identifier, category) {
    switch (category) {
        case 'names':
...
async function synthesize(text, speedAlpha, phonemizeBetweenBrackets, pauseBetweenBrackets) {
    const apiKey = process.env.RIME_API_KEY;
    if (!apiKey)
        throw new Error('RIME_API_KEY not set');
...
async function main() {
    ...
    const manifest = [];
    const categories = ['names', 'phone_numbers', 'pincodes', 'order_ids'];
    let total = 0;
    let errors = 0;
    for (const category of categories) {
        ...
        for (const identifier of items) {
            ...
            for (const [condition, result] of [['naive', naive], ['tuned', tuned]]) {
                ...
                try {
                    const audioBuffer = await synthesize(result.text, result.speedAlpha, result.phonemizeBetweenBrackets, result.pauseBetweenBrackets);
