...
export interface SynthesizeRequest {
  text: string
  speaker: string
  speedAlpha?: number        // Mist v2: > 1.0 = slower, < 1.0 = faster (inverted vs Coda)
  phonemizeBetweenBrackets?: boolean
  pauseBetweenBrackets?: boolean
  inlineSpeedAlpha?: string  // comma-separated per-word speed values
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const rimeBody: Record<string, unknown> = {
    text: body.text,
    modelId: 'mistv2',
